    ...
    public function index(Request $request)
{
    ...
    if (auth()->user()->role === 'admin') {
        ...
        if ($tahunFilter) {
            $tahunPrefix = substr($tahunFilter, 2, 2); // contoh: 2025 -> "25"
            $query->whereHas('user', function ($q) use ($tahunPrefix) {
                $q->where('nim', 'like', $tahunPrefix . '%');
            });
        }
    ...
    $groupedReports = $allReports->groupBy('user_id');

    // ✅ Perbaikan daftar tahun
    $tahunStatic = collect(range(2020, now()->year))->map(fn($y) => (string) $y);

$tahunFromDB = \App\Models\User::selectRaw("DISTINCT CONCAT('20', SUBSTR(nim, 1, 2)) as tahun")
    ->pluck('tahun')
    ->filter(function ($tahun) {
        return is_numeric($tahun) && strlen($tahun) === 4;
    });

$daftarTahun = $tahunStatic
    ->merge($tahunFromDB)
    ->unique()
    ->sortDesc()
    ->values();
    ...
    return view('semester_reports.index', compact(
        'groupedReports',
        'semesterFilter',
        'tahunFilter',
        'daftarTahun'
...
{
    // Hanya admin yang boleh mengedit laporan semester
    if (Auth::user()->role !== 'admin') {
        abort(403, 'Akses ditolak. Hanya admin yang dapat mengedit laporan semester.');
    }
...
{
    // Hanya admin yang boleh memperbarui laporan semester
    if (Auth::user()->role !== 'admin') {
        abort(403, 'Akses ditolak. Hanya admin yang dapat memperbarui laporan semester.');
    }
    ...
    $request->validate([
        ...
        'ip' => 'required|numeric|min:0|max:4',
        'ipk' => 'required|numeric|min:0|max:4',
        'khs' => 'nullable|file|mimes:pdf,jpg,jpeg,png|max:5120', // max 5MB
    ]);
    ...
    // Jika ada file KHS baru diunggah
    ...
    // Update data laporan semester
    $semesterReport->update($data);

    // Redirect dengan pesan sukses
    return redirect()->route('semester_reports.index')
