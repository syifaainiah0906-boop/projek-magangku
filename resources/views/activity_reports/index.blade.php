...
<div class="flex flex-col items-center justify-center min-h-screen p-4">
    <div class="bg-white p-8 rounded-xl shadow-lg w-full max-w-7xl">
        <h2 class="text-2xl font-semibold text-blue-900 mb-6 text-center">Laporan Kegiatan Mahasiswa</h2>

        <div class="flex items-center justify-between mb-6">
            <form action="{{ route('activity_reports.index') }}" method="GET" class="w-full max-w-lg flex items-center bg-blue-500 rounded-full overflow-hidden shadow-md">
                ...
                <tbody>
                    @forelse ($reports as $report)
                        <tr class="bg-white border-b hover:bg-gray-50">
                            ...
                            <td class="py-4 px-6">{{ $report->activity_name }}</td>
                            <td class="py-4 px-6">{{ \Carbon\Carbon::parse($report->activity_date)->isoFormat('D MMMM YYYY') }}</td>
                            <td class="py-4 px-6">{{ $report->position }}</td>
                            <td class="py-4 px-6">
                                <a href="{{ route('activity_reports.show', $report->id) }}" class="font-medium text-blue-600 hover:underline">Lihat</a>
                            </td>
                        </tr>
                    @empty
