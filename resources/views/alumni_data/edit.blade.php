    ...
    <div class="bg-white w-full max-w-3xl rounded-2xl shadow-2xl p-10 border border-blue-100">
        ...
        <form action="{{ route('alumni_data.update', $alumniDatum->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-6">
                ...
                {{-- Desa --}}
                <div class="md:col-span-2">
                    <label for="village" class="block text-sm font-medium text-gray-700">Desa</label>
                    <input type="text" name="village" id="village"
                        value="{{ old('village', $alumniDatum->village ?? '') }}"
                        class="mt-1 block w-full px-4 py-2 border-gray-300 rounded-lg shadow-sm
                ...
                <div>
                    ...
                    <select name="employment_status" id="employment_status"
                        ...
                        <option value="Bekerja" {{ old('employment_status', $alumniDatum->employment_status) == 'Bekerja' ? 'selected' : '' }}>Bekerja</option>
                        <option value="Belum Bekerja" {{ old('employment_status', $alumniDatum->employment_status) == 'Belum Bekerja' ? 'selected' : '' }}>Belum Bekerja</option>
                    </select>
                </div>
...
<div class="work-field">
    ...
    <select name="industry_field" id="industry_field"
        class="mt-1 block w-full px-4 py-2 border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 text-base">

        <option value="" disabled
            @selected(old('industry_field', $alumniDatum->industry_field) == null)>
            Pilih Bidang Industri
        </option>

        <option value="logistic"
            @selected(old('industry_field', $alumniDatum->industry_field) == 'logistic')>
            Logistic
        </option>
        <option value="agro_forestry"
            @selected(old('industry_field', $alumniDatum->industry_field) == 'agro_forestry')>
            Agro Forestry
        </option>
        <option value="energy"
            @selected(old('industry_field', $alumniDatum->industry_field) == 'energy')>
            Energy
        </option>
        <option value="technology"
            @selected(old('industry_field', $alumniDatum->industry_field) == 'technology')>
            Technology
        </option>
        <option value="education"
            @selected(old('industry_field', $alumniDatum->industry_field) == 'education')>
            Education
        </option>
        <option value="consumer"
            @selected(old('industry_field', $alumniDatum->industry_field) == 'consumer')>
            Consumer
        </option>
        <option value="investment"
            @selected(old('industry_field', $alumniDatum->industry_field) == 'investment')>
            Investment
        </option>

        {{-- Opsi input bebas --}}
        <option value="other"
            @selected(!in_array(old('industry_field', $alumniDatum->industry_field),
            ['logistic','agro_forestry','energy','technology','education','consumer','investment']))>
            Lainnya...
        </option>
    </select>

    {{-- Input manual untuk bidang industri --}}
    <input type="text" id="industry_other"
        class="hidden mt-2 block w-full px-4 py-2 border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 text-base"
        placeholder="Masukkan bidang industri lainnya"
        value="{{ in_array(old('industry_field', $alumniDatum->industry_field), ['logistic','agro_forestry','energy','technology','education','consumer','investment']) ? '' : old('industry_field', $alumniDatum->industry_field) }}">
</div>

{{-- Script mengaktifkan input bebas --}}
<script>
    const selectIndustry = document.getElementById('industry_field');
    const inputOther = document.getElementById('industry_other');
    
    function toggleIndustryInput() {
        if (selectIndustry.value === 'other') {
            inputOther.classList.remove('hidden');
            inputOther.name = 'industry_field'; // ganti ke nama utama
            inputOther.focus();
        } else {
            inputOther.classList.add('hidden');
            inputOther.name = 'industry_other'; // biar tidak override
        }
    }

    // Jalankan saat load & saat berubah
    toggleIndustryInput();
    selectIndustry.addEventListener('change', toggleIndustryInput);
</script>

                {{-- Alamat Kantor --}}
                <div class="md:col-span-2 work-field">
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        statusSelect.addEventListener('change', toggleWorkFields);
    });
</script>
@endpush
